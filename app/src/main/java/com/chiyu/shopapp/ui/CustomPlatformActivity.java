...
public class CustomPlatformActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		url = URL.FENXIANG_DEBUG+companyId+"/line/line.detail.html?";
...
//			shareToEmail();
	...
	private void sendSMS(String webUrl) {
		String smsBody = title+ url+"lineid="+lineid+"&dateid="+dateid;
...
//		sendIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, title);
	...
	private void sendMail(String emailUrl) {
		...
		String emailBody =  url+"lineid="+lineid+"&dateid="+dateid;
		// 邮件主题
	...
	private void shareToQQ() {
		new ShareAction(this).setPlatform(SHARE_MEDIA.QQ)
				...
				.withTargetUrl(url+"lineid="+lineid+"&dateid="+dateid).withMedia(IMAGE)
				.share();
	}
...
//	}
	...
	private void shareToQQCircle() {
		new ShareAction(this).setPlatform(SHARE_MEDIA.QZONE)
				.setCallback(umShareListener).withText(title).withMedia(IMAGE)
		.withTargetUrl(url+"lineid="+lineid+"&dateid="+dateid)
		.share();
	}
...
//	}
	...
	private void shareToWeiXinCircle() {
		...
		new ShareAction(this).setPlatform(SHARE_MEDIA.WEIXIN_CIRCLE)
				.setCallback(umShareListener).withTitle(title).withText(title)
				.withTargetUrl(url+"lineid="+lineid+"&dateid="+dateid).withMedia(IMAGE)
				.share();
	}
	...
	private UMShareListener umShareListener = new UMShareListener() {
		...
		public void onResult(SHARE_MEDIA platform) {
			...
			Toast.makeText(CustomPlatformActivity.this, platform + " 分享成功啦",
					Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onError(SHARE_MEDIA platform, Throwable t) {
			Toast.makeText(CustomPlatformActivity.this, platform + " 分享失败啦",
					Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onCancel(SHARE_MEDIA platform) {
			Toast.makeText(CustomPlatformActivity.this, platform + " 分享取消了",
					Toast.LENGTH_SHORT).show();
		}
	};
